...
while cam.isOpened():
    _, frame1 = cam.read()  # frame 1
    _, frame2 = cam.read()  # frame 1
    diff = cv2.absdiff(frame1, frame2)  # get the different between the frames
    gray = cv2.cvtColor(diff, cv2.COLOR_RGB2GRAY) # turn them to gray for reducing the cpu needs (as no need color for detection)
    blur = cv2.GaussianBlur(gray, (5, 5), 0)  # blurs for reducing the cpu needs
    ...
    # cv2.drawContours(frame1 , contours , -1, (0,255,0), 2 ) # uncomment if u want to see the contours
    num = 0
    for c in contours:
        num = num + 1
        ...
        cv2.rectangle(frame1, (x, y), (x + w, y + h), (0, 255, 0),
                      2)  # (G, B, R) # draw rectangle in the bigger contours
        # winsound.Beep(500, 200)
        print("there is a motion detected")
        # sent notify (in the app)
        break
    imgStack = stackImages(.5, (
    [frame1, diff, gray], [blur, thresh, dilated]))  # stacked all processing steps in the frames to demonstrate
    # print(num)
